...
public class MC1Engine extends BaseEngine
{
    ...
    {
        playouts();
        final short move = findBestFor(game.playerToPlay());
    ...
    private short findBestFor(Color playerToPlay)
    {
        final Game copy = gameBuffers.lease();
        try
        {
            short[] tops = ttable.topsFor(game.getBoard().hashCode(), boardSize * boardSize, playerToPlay);
            for (int i = 0; i < tops.length; i++)
            {
                ...
                {
                    short move = Move.move(coord, playerToPlay);
                    game.copyTo(copy);
                    if (copy.play(move))
                    {
